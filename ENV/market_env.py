...
class MarketEnv(gym.Env):
	...
	def _step(self, action):
		...
		if self.done:
			...
			self.boughts = []
		tmp = []
		tmp.append(self.state[0])
		tmp.append(self.state[1])
		tmp.append(self.reward)
		tmp.append(self.done)
		return self.state[0], self.state[1],self.reward, self.done, {"dt": self.targetDates[self.currentTargetIndex], "cum": self.cum, "code": self.targetCode}
	...
	def defineState(self):
		...
		#tmpState = np.array([float(i) for i in str(tmpState).replace('[', '').replace(' ', '').replace(']', '').split(",")])
		self.state = tmpState

'''
class MarketPrint:

	def printStep(info,actions,action,reward_sum): # actions: list of env.actions
		if actions[action] == "LONG" or actions[action] == "SHORT":
			color = bcolors.FAIL if actions[action] == "LONG" else bcolors.OKBLUE
